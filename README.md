...
5. **Reconfigure or remove it.** Running extensions can be updated (`POST /api/extensions/reconfigure`, `PATCH /api/extensions/:id`) or withdrawn (`DELETE /api/extensions/:id`, `POST /api/extensions/revoke`) via the UI settings-style dialog directly in the room (authorized seamlessly via the in-room session token) or via the API with the host token (`x-voicebox-host-token`), updating bounds or revoking tools without discovering or supplying hidden `.token` files.
