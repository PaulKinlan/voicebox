...
// ── ONE HOME for the unimplemented-adapter sentence (voicebox-beads-aaj review note 1, y5k class) ──
// The ADMISSION path (boot table + /api/agents) uses the label-aware sentence. The DELEGATE
// path for claude keeps its long-standing, test-pinned sentence — a user-facing contract that
// outlives this module. Both sentences live HERE so the next reader changes one home, not two.
export function unimplementedAdapterWhy(label) {
  return `No Voicebox task adapter is implemented for '${label}' on this host; the agent is configured but cannot run.`;
}
export const CLAUDE_LEGACY_DELEGATE_WHY = "No Voicebox task adapter is configured for this CLI; configure an adapter before delegating.";
...
export function describeAgentAdmission(agent, { describeAdapter, implementedAdapters = new Set(["pi-acp"]), executorSelected = true }) {
  ...
  if (!implementedAdapters.has(adapter)) {
    return {
      ...
      why: unimplementedAdapterWhy(adapter),
    };
  }
 ...
 *     the server binds describeAdapterInstall; the agent argument is accepted (and today
 *     unused) because the configured-agent schema has no per-agent adapterDir — the adapter
 *     install is a HOST fact, described once, not per agent.
