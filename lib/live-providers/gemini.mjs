...
export function createGeminiProvider({ model, emit, log, transport, tools, systemInstruction }) {
  ...
  async function onFrame(event) {
    ...
    if (msg.toolCall) {
      const calls = (msg.toolCall.functionCalls ?? []).map((c) => ({ id: c.id, name: c.name, args: c.args ?? {} }));
      if (calls.length) emit({ type: "tool-call", calls });
      return;
    }
