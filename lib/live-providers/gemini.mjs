...
export function createGeminiProvider({ model, emit, log }) {
  ...
  ws.onerror = (e) => emit({ type: "error", message: e?.message ?? "upstream websocket error" });
  ws.onclose = (e) => emit({ type: "closed", code: e.code, reason: e.reason });
