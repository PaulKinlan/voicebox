...
export function createOpenAIProvider({ model, emit, log, transport, tools, systemInstruction, instruction, projectInstruction, voice, debug }) {
  const key = process.env.OPENAI_API_KEY;  if (!key) throw new Error("OPENAI_API_KEY is not set — the OpenAI Realtime session cannot start");
  ...
  function sendToolResponse(responses) {
    for (const { id, response } of responses) {
      ...
      debug?.({ type: "tool.delivery", callId: id, response, delivery: sent ? "transport-accepted" : "not-sent", modelReceipt: "unknown", severity: sent ? "info" : "error" });
    ...
    onEvent: (ev) => {
      if (ev.kind === "open") {
        ...
        transport.send("handshake", JSON.stringify({
          ...
          session: {
            ...
            ...(instruction || systemInstruction || projectInstruction ? { instructions: [instruction, projectInstruction, systemInstruction].filter(Boolean).join("\n\n") } : {}),
            audio: {
      ...
      switch (msg.type) {
        ...
        case "response.function_call_arguments.done": {
          debug?.({ type: "tool.wire-request", callId: msg.call_id, name: msg.name, arguments: msg.arguments });
          if (typeof msg.call_id !== "string" || !msg.call_id.trim()) {
            debug?.({ type: "tool.dropped", severity: "error", reason: "missing-call-id", delivery: "not-sent" });
          ...
          try {
            ...
            debug?.({ type: "tool.refused", callId: msg.call_id, name: msg.name, reason: "invalid-tool-call", route: "provider-validation" });
