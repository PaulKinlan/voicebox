...
export function createBrowserTaskHost(options = {}) {
  ...
  async function admit(args, authority) {
    ...
    // 1. Capture root synchronously before ANY await
    const selected = root();
    if (!selected) {
    ...
    // 2. Validate executor and capture bounds synchronously before ANY await
    ...
    // 3. Synchronously check/register in-flight admission promise BEFORE any await
    ...
    // 4. Check for prior completed/admitted task with this callId
