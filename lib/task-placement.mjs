...
//   1. Full-key cryptographic authentication: every address/record lookup verifies full-key HMAC without leaking key bytes in locators.
//   2. Tuple identity: distinct (owner, callId) pairs use null separator (\0); foreign owners cannot read or share records.
//   3. Simultaneous call-ID deduplication: pending calls await the same admission promise; retries execute once.
//   4. Immutability: root descriptor and bounds captured synchronously before async signing window.
//   5. Durable persistence gate: storage failures fail closed; call index is derivable from persisted task records.
//   6. Reload call-ID recovery: durable callId mapping survives page reload; repeated callId reuses existing task.
//   7. Consistent liveness / staleness: unconfirmed tasks from prior boot generations report stale=true across both status and retry.
//   8. Alive capacity retention: timed-out runners retain capacity allocation until runner promise actually settles.
//   9. Outcome contract: terminal states carry outcome class and basis (claimed-complete / executor-claimed).
...
export function createBrowserTaskHost(options = {}) {
  ...
  const keyTag = computeKeyTag(keyBytes);
  ...
  function scopedCallKey(owner, callId) {
    return `${environment}\0${keyTag}\0${owner}\0${callId}`;
  }
  ...
  async function authenticateAddress(address, owner) {
    if (typeof address !== "string" || address.length > 8192) return false;
    const match = /^task_([A-Za-z0-9_-]+)\.([0-9a-f]{64})$/.exec(address);
    if (!match) return false;
    const expectedSig = await seal(match[1], owner);
    if (match[2] !== expectedSig) return false;
    try {
      const loc = JSON.parse(base64UrlDecode(match[1]));
      if (loc.environment !== environment) return false;
    } catch {
      return false;
    }
    return true;
  ...
  async function retrieveCall(owner, callId) {
    const key = scopedCallKey(owner, callId);
    let addr = callRecords.get(key) ?? globalMemoryCalls.get(key);
    if (!addr && hasLocalStorage()) {
      try {
    ...
    if (addr) {
      if (await authenticateAddress(addr, owner)) {
        const task = retrieveTask(addr);
        if (task && task.owner === owner && task.environment === environment) {
          return task;
        }
      }
    }
    // Derivable index fallback: search stored tasks for matching owner, callId, environment
    for (const task of tasks.values()) {
      if (task.callId === callId && task.owner === owner && task.environment === environment) {
        if (await authenticateAddress(task.address, owner)) {
  ...
  function viewForRecord(record) {
    ...
    const p = latestProgress.get(record.address);
  ...
  async function cancelTask(address, owner, graceMs = 1500) {
    ...
    const record = retrieveTask(address);
  ...
  async function runTask(address, record, implementation, executionPromise, capturedBounds) {
    ...
    try {
      ...
      record.updatedAt = new Date().toISOString();
      persistTaskQuiet(record);
      try { onUpdate?.(viewForRecord(record)); } catch {}
    } catch (err) {
      clearTimeout(timer);
  ...
  async function admit(args, authority) {
    ...
    // 1. Capture root and executor synchronously before ANY await so caller mutations cannot drift
    const currentRoot = root();
    const capturedRoot = currentRoot ? Object.freeze(JSON.parse(JSON.stringify(currentRoot))) : null;

    const implementation = executor();
    let executorCheck = null;
    if (implementation && typeof implementation.check === "function" && typeof implementation.run === "function") {
      ...
      if (checked && typeof checked.then === "function") {
        ...
        executorCheck = refusal("unbounded-executor", "task admission requires a synchronous host boundary check");
      } else if (!checked || checked.ok !== true) {
        executorCheck = { ok: false, refused: checked?.refused ?? "unbounded-executor", why: checked?.why ?? "executor check failed" };
      } else {
        const boundCheck = validatePlacementBounds("browser", checked.bounds);
        if (!boundCheck.ok) {
          executorCheck = boundCheck;
        } else if (typeof checked.mechanism !== "string" || !checked.mechanism) {
          executorCheck = refusal("unbounded-executor", "executor check did not establish a host enforcement mechanism");
        } else {
          executorCheck = {
            ok: true,
    ...
    // 2. Synchronously check/register in-flight admission promise BEFORE any await
    ...
    if (inflight) {
      ...
      return { ok: true, task: viewForRecord(existing.record), existing: true };
    }

    let resolvePending;
    const admissionPromise = new Promise((resolve) => {
      resolvePending = resolve;
    });
    pendingCalls.set(callKey, admissionPromise);

    // 3. Check for prior completed/admitted task with this callId
    const existing = await retrieveCall(authority.owner, authority.callId);
    if (existing) {
      pendingCalls.delete(callKey);
      if (existing.owner !== authority.owner) {
        const err = refusal("task-owner-mismatch", "call id belongs to another owner");
        resolvePending(err);
        return err;
      }
      if (JSON.stringify(existing.input) !== JSON.stringify(parsed.value)) {
        const err = refusal("task-call-id-conflict", "that call id already admitted different task content");
        resolvePending(err);
        return err;
      }
      const res = { ok: true, record: existing, task: viewForRecord(existing), existing: true };
      resolvePending(res);
      return { ok: true, task: res.task, existing: true };
    }

    // 4. For fresh executions: enforce root, executor availability and bounds from the synchronous snapshot
    if (!capturedRoot) {
      pendingCalls.delete(callKey);
      const err = refusal("task-root-unavailable", "task delegation requires a declared root descriptor; none declared");
      resolvePending(err);
      return err;
    }
    ...
    if (!implementation) {
      pendingCalls.delete(callKey);
      const err = refusal("executor-unavailable", `no admitted task executor for '${parsed.value.agent}' in browser placement`);
      resolvePending(err);
      return err;
    }
    ...
    if (typeof implementation.check !== "function" || typeof implementation.run !== "function") {
      pendingCalls.delete(callKey);
      const err = refusal("unbounded-executor", `no host-enforced executor for '${parsed.value.agent}'`);
      resolvePending(err);
      return err;
    }

    if (!executorCheck || !executorCheck.ok) {
      pendingCalls.delete(callKey);
      const err = executorCheck || refusal("unbounded-executor", "executor check failed");
      resolvePending(err);
      return err;
    }

    const capturedBounds = executorCheck.bounds;

    // 5. Atomic synchronous capacity reservation
    if (activeCount >= PLACEMENT_BOUNDS.browser.maxActiveTasks) {
      pendingCalls.delete(callKey);
      const err = refusal("task-capacity-exhausted", `browser environment capacity exhausted (${PLACEMENT_BOUNDS.browser.maxActiveTasks} max active tasks)`);
      resolvePending(err);
      return err;
    }
    activeCount++;
    ...
    try {
      ...
      activeCount = Math.max(0, activeCount - 1);
      pendingCalls.delete(callKey);
      resolvePending(refusal("task-admission-failed", e?.message ?? String(e)));
      throw e;
    }
    ...
    const record = {
      ...
      mechanism: executorCheck.mechanism,
      boot,
      state: "queued",
      createdAt: now,
      updatedAt: now,
    ...
    // 6. Durable persistence MUST succeed before authorizing execution
    try {
    ...
    const successResult = { ok: true, record, task: viewForRecord(record), existing: false };
    ...
    async call(tool, args, authority) {
      ...
      try {
        ...
        if (tool === "task_status") {
          ...
          return { ok: true, task: viewForRecord(record) };
        }
        if (tool === "cancel_task") {
