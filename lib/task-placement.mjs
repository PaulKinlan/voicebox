...
//
// Invariants enforced (voicebox-beads-8fv.1 revision items):
...
//   3. Immutability: root descriptor and bounds captured before async signing window.
//   4. Durable persistence gate: storage quota/write failures fail closed with task-persistence-failed and prevent execution.
//   5. Reload call-ID recovery: durable callId mapping survives page reload; repeated callId reuses existing task.
//   6. Lost runner reconciliation: non-terminal tasks from a dead generation (boot !== currentBoot) reconcile to interrupted.
...
export function createBrowserTaskHost(options = {}) {
  ...
  const pendingCalls = new Map(); // `${owner}\0${callId}` -> Promise for concurrent in-flight admission
  ...
  function reconcilePriorBoot(record) {
    if (!record || taskTerminal(record.state)) return record;
    if (record.boot !== boot && !activeRuns.has(record.address)) {
      // The prior runner belongs to a dead browser session/generation;
      // reconcile to interrupted rather than pretending it is still running.
      record.state = "interrupted";
      record.reason = "environment-ended-outcome-unknown";
      record.outcome = outcomeFor("interrupted");
  ...
  async function cancelTask(address, owner, graceMs = 1500) {
    ...
    record = reconcilePriorBoot(record);
  ...
  async function admit(args, authority) {
    ...
    const callKey = `${authority.owner}\0${authority.callId}`;
    ...
    if (inflight) {
      ...
      if (JSON.stringify(existing.task.input) !== JSON.stringify(parsed.value)) {
    ...
    const successResult = { ok: true, task: taskView(record), existing: false };
    ...
    async call(tool, args, authority) {
      ...
      try {
        ...
        if (tool === "task_status") {
          ...
          let record = retrieveTask(args.address);
