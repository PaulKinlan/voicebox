...
function recordAdmission(id) {
  mkdirSync(EXTENSIONS_DIR, { recursive: true });
  appendFileSync(LEDGER_FILE, JSON.stringify({ id, at: new Date().toISOString(), decision: "admitted" }) + "\n");
}
...
export function admitProposal(id, decision = "admit") {
  ...
  renameSync(tmp, dest);
  recordAdmission(id); // the ledger: set membership is recorded admissions, not directory contents
...
export function inventory() {
  ...
  const present = [...presentNotAdmitted].map((id) => {
    try {
