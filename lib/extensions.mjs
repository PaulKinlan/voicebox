...
import { admit, PRIMITIVE_NEEDS, PRIMITIVES } from "../core/extensions.ts";
...
// The state directories have ONE owner (`lib/state-dirs.mjs`) and this module asks for them. It used
// to resolve them itself — twice, differently, five lines apart (voicebox-beads-y5k).
import { REPO, workspaceDir, extensionsDir, wasmShelfDir } from "./state-dirs.mjs";
...
const EXTENSIONS_DIR = extensionsDir(); // the host's directory — outside the root
...
export function validateExtensionDescriptor(descriptor) {
  if (!descriptor || typeof descriptor !== "object" || Array.isArray(descriptor)) {
    return { ok: false, refused: "bad-descriptor", why: "an extension descriptor must be an object" };
  }
  const id = String(descriptor.id ?? "");
  if (!/^[a-z0-9_-]+$/.test(id)) {
    return { ok: false, refused: "invalid-id", why: "extension id must match /^[a-z0-9_-]+$/" };
  }
  if (!descriptor.name || typeof descriptor.name !== "string") {
    return { ok: false, refused: "missing-name", why: "extension requires a non-empty name" };
  }
  if (!descriptor.description || typeof descriptor.description !== "string") {
    return { ok: false, refused: "missing-description", why: "extension requires a description" };
  }
  if (!Array.isArray(descriptor.tools) || descriptor.tools.length === 0) {
    return { ok: false, refused: "no-tools", why: "extension requires at least one tool in 'tools'" };
  }
  for (const t of descriptor.tools) {
    if (!t.name || !/^[a-z0-9_]+$/.test(t.name)) {
      return { ok: false, refused: "bad-tool-name", why: `tool name '${t?.name}' must match /^[a-z0-9_]+$/` };
    }
    if (!t.primitive || !PRIMITIVES.includes(t.primitive)) {
      return { ok: false, refused: "unknown-primitive", why: `tool '${t.name}' uses unknown primitive '${t?.primitive}'` };
    }
  }
  return { ok: true, id };
}

export function createExtension(descriptor, source = "local") {
  const valid = validateExtensionDescriptor(descriptor);
  if (!valid.ok) return valid;
  const desc = {
    ...descriptor,
    source,
    runsIn: descriptor.runsIn ?? "host",
    capabilities: descriptor.capabilities ?? [],
    bounds: descriptor.bounds ?? {},
  };
  const prop = propose(desc, source);
  if (!prop.ok) return prop;
  const plan = planFor(desc, existingNamesExcept(desc.id));
  return {
    ok: true,
    id: desc.id,
    state: "pending",
    plan,
    note: "staged as a pending local extension — review the plan and admit with host authority",
  };
}

export function createAndAdmitExtension(descriptor, actor = "host") {
  const valid = validateExtensionDescriptor(descriptor);
  if (!valid.ok) return valid;
  const desc = {
    ...descriptor,
  ...
  const gate = admit(desc, PLACEMENT, existingNamesExcept(desc.id));
  if (gate.decision !== "admitted") {
    return { ok: false, refused: gate.rule, why: gate.why, plan: planFor(desc) };
  }
  propose(desc, desc.source);
  return admitProposal(desc.id, "admit", actor);
}
...
export function proposalPlan(id) {
  ...
  // The plan is about THIS id: its own loaded tool names are not collisions with
  // themselves, or a re-admission would be disclosed as a duplicate-tool refusal
  // while the admit door below (n0n) admits it.
  return { ...planFor(record, existingNamesExcept(safeId)), state: record.state, refusal: record.refusal ?? null };
}

export function cataloguePlan(id) {
  if (typeof id === "string" && id.startsWith("wasm-shelf-")) {
    const shelfDir = wasmShelfDir();
...
const PENDING_APPROVAL_FILE = () => path.join(EXTENSIONS_DIR, ".pending-approval.json");
...
export function admitProposal(id, decision = "admit", actor = undefined, reviewedPlan = undefined) {
  ...
  if (reviewedPlan) {
    const currentPlan = { ...planFor(record, existingNamesExcept(safeId)), state: fromHostDir ? "present-not-admitted" : record.state, refusal: record.refusal ?? null };
  ...
  // The candidate's OWN tool names are excluded from the collision set: re-admitting a
  // changed descriptor for an already-loaded extension is an UPDATE, not a tool colliding
  // with itself. Without this, every re-admission of the same id refused 'duplicate-tool'
  // and the registry kept the stale descriptor — the n0n disparity: a corrected params.url
  // never reached the wire (driven 2026-09-25).
  const gate = admit(record, PLACEMENT, existingNamesExcept(safeId));
  ...
  renameSync(tmp, dest);
  // The ledger: set membership is recorded admissions, not directory contents. A
  // re-admission of an already-admitted id is recorded as 'reconfigured' so the history
  // keeps first admission and update distinguishable — both count as admitted, and a
  // later 'revoked' removes either.
  if (actuallyAdmitted && !fromHostDir) {
    appendLedger({ id: safeId, at: new Date().toISOString(), decision: "reconfigured", descriptor: record });
  } else {
    recordAdmission(safeId);
  }
  if (!fromHostDir) {
...
export function reconfigureExtension(id, updates = {}, actor = undefined) {
  ...
  // Parameter updates are addressed BY TOOL NAME: callHttp reads tools[i].params, so a
  // descriptor-root `params` would be an ignored no-op (n0n review finding). Shape:
  //   { params: { <toolName>: { url, headers, ... } } }
  if (updates.params !== undefined) {
    if (!updates.params || typeof updates.params !== "object" || Array.isArray(updates.params)) {
      const why = "params must be an object keyed by tool name: { <toolName>: { ... } }";
      audit({ kind: "import", target: `extensions/${safeId}`, tool: "extensions.reconfigure" }, "refuse", "params-invalid", "refused", null, why, actor);
      return { ok: false, refused: "params-invalid", why };
    }
    const tools = updated.tools ?? [];
    const known = new Set(tools.map((t) => t.name));
    for (const [name, overlay] of Object.entries(updates.params)) {
      if (!known.has(name)) {
        const why = `params names tool '${name}', which this extension does not carry — it carries: ${[...known].join(", ") || "no tools"}`;
        audit({ kind: "import", target: `extensions/${safeId}`, tool: "extensions.reconfigure" }, "refuse", "params-unknown-tool", "refused", null, why, actor);
        return { ok: false, refused: "params-unknown-tool", why };
      }
      if (!overlay || typeof overlay !== "object" || Array.isArray(overlay)) {
        const why = `params for tool '${name}' must be an object of parameters`;
        audit({ kind: "import", target: `extensions/${safeId}`, tool: "extensions.reconfigure" }, "refuse", "params-invalid", "refused", null, why, actor);
        return { ok: false, refused: "params-invalid", why };
      }
    }
    updated.tools = tools.map((tool) => {
      const overlay = updates.params[tool.name];
      if (!overlay) return tool;
      return { ...tool, params: { ...(tool.params ?? {}), ...overlay } };
    });
  }
...
function loadRegistry() {
  ...
  // THE FILE THAT VANISHED (review finding, voicebox-beads-qdo): the loop above walks FILES, so
  // an admitted id whose descriptor file was deleted lands in NO list — not running (nothing
  // loaded), not present (the ledger still admits it), not failed (there was no file to fail on).
  // Diff the recorded admissions against what the walk accounted for: an admitted id with no
  // file and no failure is a NAMED failure too — the silent half-load, closed at the seam.
  const accounted = new Set(loadFailures.keys());
  for (const entry of registry.values()) accounted.add(entry.descriptorId);
  for (const id of admitted) {
    if (accounted.has(id)) continue;
    const file = path.join(EXTENSIONS_DIR, `${id}.json`);
    loadFailures.set(id, {
      id,
      refused: "descriptor-missing",
      why: "the recorded admission has no descriptor file — the file was deleted or never written",
      next: `restore ${file} (re-sideload it from the catalogue or your backup), or revoke the extension to clear the recorded admission`,
    });
    console.error(`[extensions] '${id}' is admitted but its descriptor file is GONE — NOT loaded (descriptor-missing). next: restore ${file} or revoke`);
  }
}
