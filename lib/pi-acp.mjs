 ...
 */
export function describeAdapterInstall({ adapterDir: rawAdapterDir } = {}) {
  const fallback = path.join(os.homedir(), ".pi/agent/npm/node_modules/pi-acp");
  const adapterDir = rawAdapterDir ?? process.env.VOICEBOX_ACP_ADAPTER ?? fallback;
  let resolved;
  ...
  catch { return { ok: false, refused: "adapter-unavailable", why: "pi-acp package.json is unreadable.", adapterDir: resolved }; }
  if (pkg.name !== ACP_AGENT.name || pkg.version !== ACP_AGENT.version) {
    return { ok: false, refused: "adapter-version-unsupported", why: `requires installed ${ACP_AGENT.name} ${ACP_AGENT.version}`, adapterDir: resolved, installedVersion: pkg.version ?? null };
  }
  const entry = path.join(resolved, "dist", "index.js");
  if (!fs.existsSync(entry)) {
    return { ok: false, refused: "adapter-unavailable", why: "pi-acp dist/index.js missing; build or install the adapter package.", adapterDir: resolved };
  }
...
export function createPiAcpExecutor(options = {}) {
  ...
  function checkAdapter() {
    const described = describeAdapterInstall({ adapterDir, piBinary });
