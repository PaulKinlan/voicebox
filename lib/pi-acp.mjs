...
export function createPiAcpExecutor(options = {}) {
  ...
  function check(args = {}) {
    if (args.input?.agent && args.input.agent !== "pi" && args.input.agent !== "pi-acp") {
      if (args.input.agent === "claude") {
      ...
      return { ok: false, refused: "adapter-not-configured", why: `No Voicebox task adapter is configured for '${args.input.agent}'.` };
    }
    ...
    return {
      ...
      bounds: {
        deadlineMs: Math.min(timeoutMs, 60000),
        maxOutputBytes: Math.min(maxOutputBytes, 65536),
      },
    };
  }
