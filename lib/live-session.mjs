...
export function resolvedLiveProviderName(explicit) {
  ...
  return explicit ?? process.env.LIVE_PROVIDER ?? "gemini";
}
 ...
 */
export function createLiveSession({ onAudioOut, onText, onState, onToolCall, log = console.error, provider, model, tools, systemInstruction } = {}) {
  ...
  const emit = (event) => {
    ...
    if (event.type === "tool-call") { onToolCall?.(event.calls); return; }
  ...
  try {
    inner = create({ model, emit, log, transport, tools, systemInstruction });
  } catch (err) {
  ...
  return {
    ...
    sendToolResponse(responses) {
      if (state.closed || !state.ready) return false;
      ...
      return inner.sendToolResponse(responses);
    },
