  ...
  return {
    ...
    connect(url, next = {}) {
      ...
      try {
        ...
        throw new Error(
          `live transport: this provider asked for connection headers, and the real dial with those headers ` +
            `attached was REFUSED by this runtime — the probe is the dial itself, not a guess about the ` +
            `runtime. WebSocket constructor said: ${e?.message ?? String(e)}`,
        );
      }
