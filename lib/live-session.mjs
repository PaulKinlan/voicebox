...
export function createLiveSession({ onAudioOut, onText, onState, log = console.error, provider, model } = {}) {
   ...
   * It notifies directly rather than through `emit` — the first version set `state.closed` and then
   * emitted, so the dedupe it had just installed swallowed its own terminal event. One place to enter
   * the state, one place to say so.
   */
  const terminate = (code, reason) => {
    if (state.closed) return;                     // exactly one terminal event, whatever the cause
    state.closed = true;
    state.ready = false;                          // the truthful state machine: ready never survives
    transport.close();                            // a terminal state must not leave a socket behind
