...
//
// WHAT THIS DOES NOT BOUND (vb-resolver's review, all three driven with digest-consistent attack
// modules, 2026-09-23): TIME — the export runs SYNCHRONOUSLY on the host's event loop and wasm has
// no fuel, so an infinite loop hangs the call (and with it, the server); MEMORY — memory.grow
// needs no import, so allocation is unbounded. AN ADMITTED MODULE IS TRUSTED FOR ITS TIME AND
// MEMORY: NEITHER IS BOUNDED. The digest binds BYTES, never behavior. The fix is a worker with a
// timeout and a memory cap (follow-up bead); until then this header and the gate's admission plan
// are the honest surface — and nothing here may be called 'containment' of anything but
// CAPABILITIES (the zero-import closure), never of resources.
//
...
export async function callWasmTool(tool, args = {}) {
  ...
  // The declared addresses are TRUSTED input, not verified fact (vb-resolver's review, driven:
  // a zero-page module crashed this driver with an UNCAUGHT RangeError). Check the memory covers
  // everything the ABI declares BEFORE touching it — a lie about addresses is a named refusal.
  ...
  if (memory.buffer.byteLength < needed) {
    return { ok: false, refused: "unsupported-abi", why: `the module's memory is ${memory.buffer.byteLength} bytes but the declared ABI needs ${needed} (input at ${spec.input.addr}+${spec.input.maxBytes}, output at ${spec.output.addr}+${spec.output.bytes}) — the bytes are not buffer-abi/1` };
  }
