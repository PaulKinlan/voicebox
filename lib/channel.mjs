 ...
 */
export function createChannel({ peer, connected, send, timeoutMs = 5000, environment }) {
  ...
  return {
    ...
    ask(parts) {
      /**
       * **A channel that cannot say who it is does not get to ask.**
       *
       * The environment key is the channel's own identity (`createChannel`'s
       * `environment`), not something an asker supplies: it is spread LAST over
       * the caller's parts so a tool call cannot claim to be another host. A
       * call with no actor could only be refused positionally — "not reachable
       * from here", whose "here" two machine environments read about
       * themselves — so it is refused here, by name, before it is sent.
       */
      if (typeof environment !== "string" || environment === "") {
        return Promise.resolve({
          ...
          why:
            "this channel does not know which environment it is, so a call it sends would name no actor; " +
            "the harness constructs the channel with its own environment key (`createChannel({ environment })`)",
        });
      }
      if (!connected()) {
      ...
      const envelope = makeCall({ callId: callId(), ...parts, environment });
...
export function createExecutorDoor({ lookup, exec }) {
  return {
    ...
    receive(raw) {
      // ONE normalisation, not two paths: the wire carries strings, so a call
      // held as an object (the in-process framing) is stringified ONCE here,
      // and everything downstream — the callId probe and parseCall — sees the
      // same bytes. (Review finding, isocan-flash 2026-09-19: parseCall tested
      // the uninitialised obj instead of raw, so objects were refused as
      // malformed; the door's parse-a-copy-but-forward-the-original existed to
      // work around it and collapsed once the word was fixed.)
      const wire = typeof raw === "string" ? raw : JSON.stringify(raw);
      let probe = null;
      try {
        probe = JSON.parse(wire);
      } catch {
        return null;
      }
      const callId = typeof probe?.callId === "string" ? probe.callId : null;
      const parsed = parseCall(wire, lookup);
      if (!parsed.ok) {
      ...
      if (result instanceof Promise) {
        // ASYNC THROWS GET AN ANSWER TOO (journal-omr, the page-side half). A sync throw is
        // caught below, but a rejection inside an awaited executor used to escape the caller:
        // the page's socket callback awaited receive(), the rejection became an unhandled
        // promise, and NOTHING went back on the wire — so the asker waited out its timeout,
        // which is the honest answer to a silent peer but not to a page that threw. The same
        // principle as the server's `answerOnce`.
        return result
          .then((done) => JSON.stringify(toAnswer(callId, done)))
          .catch((e) => JSON.stringify(makeAnswerRefused(callId, "exec-threw", e?.message ?? String(e))));
      }
