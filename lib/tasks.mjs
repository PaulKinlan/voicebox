...
export function createTaskHost({ environment, instance, boot, addressKey, root, executor = () => installedExecutor }) {
  ...
  function settle(loc, record, event) {
    return append(loc, { address: record.address, ...event }, record.project, record.callId);
  }
  ...
  async function run(loc, record, implementation) {
    ...
    try {
      ...
      try {
        execution = Promise.resolve().then(() => implementation.run({ input, bounds, signal: running.controller.signal }));
      ...
      settle(loc, record, { state: "completed", answer });
    } catch (err) {
      // D6 owns public Stop/cancel escalation. A deadline is not proof that a subprocess stopped.
      const state = err instanceof TaskInterrupted || err.refused === "task-deadline" ? "interrupted" : "failed";
      try { settle(loc, record, { state, reason: err.refused ?? "executor-failed" }); }
  ...
  function admit(args, authority) {
    ...
    const checked = implementation.check({ input: parsed.value, root: Object.freeze({ ...selected.root }), environment });
    ...
    const record = {
      address, environment, owner: authority.owner, root: { kind: "machine", path: canonical, environment },
      project: selected.project, instance, callId: authority.callId, input: parsed.value,
      bounds: { deadlineMs: checked.bounds.deadlineMs, maxOutputBytes: checked.bounds.maxOutputBytes }, mechanism: checked.mechanism, boot, pid,
  ...
  return {
    call(tool, args, authority) {
      ...
      try {
        ...
        return { ok: true, task: taskView(recordAt(loc, args.address, authority.owner)) };
      } catch (err) {
