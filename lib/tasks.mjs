...
export function createTaskHost({ environment, instance, boot, addressKey, root, executor = () => installedExecutor, agentRegistry = null }) {
  ...
  async function run(loc, record, implementation) {
    ...
    try {
      ...
      try {
        execution = Promise.resolve().then(() => implementation.run({ input, bounds, signal: running.controller.signal, report, root: record.root }));
  ...
  function admit(args, authority) {
    ...
    if (agentRegistry) {
      ...
      if (parsed.value.agent.startsWith("agent_") && !configuredAgent) {
        return refusal("agent-not-configured", `No configured agent with ID '${parsed.value.agent}' exists.`);
      }
      if (configuredAgent) {
