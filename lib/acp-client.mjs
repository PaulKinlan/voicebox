...
export const ACP_AGENT = Object.freeze({ name: "pi-acp", version: "0.0.34", piVersion: "0.87.1" });
...
export function createAcpClient(transport, { timeoutMs = 10000, maxBytes = 65536, decide } = {}) {
  ...
  let next = 0, stopped = null, initialized = false, session = null, prompt = null, turns = 0, cancelling = false;
  ...
  return {
    ...
    async setConfigOption(configId, value) {
      if (!session) throw fail("acp-state", "one active session is required to set configuration options");
      if (typeof configId !== "string" || !configId) throw fail("acp-invalid-config", "configId must be a non-empty string");
      if (typeof value !== "string") throw fail("acp-invalid-config", "config value must be a string");
      return await request("session/set_config_option", { sessionId: session, configId, value });
    },
    async prompt(text) {
      ...
      try {
        ...
        /**
         * **A turn the harness reports as CANCELLED is settled by name, not
         * failed as incomplete** (`voicebox-beads-6co`). The two are different
         * facts: `acp-turn-incomplete` means the harness answered something
         * this client does not accept; `task-cancelled` means the turn stopped
         * because a cancel was sent. Collapsing them would make a cancel look
         * like a fault, and the text collected before the stop is not an answer.
         */
        if (result?.stopReason === "cancelled") {
          throw fail("task-cancelled", "the harness reported this turn cancelled; the text collected before it stopped is not a completed answer");
        }
    ...
    /**
     * **Cancel a running turn, or refuse BY NAME.** Three states, three names,
     * because "there is nothing to cancel" has two different remedies: start a
     * task, or stop trying to cancel one that has already finished.
     *
     *   task-not-found      — no task has run in this session (nothing to cancel)
     *   task-not-running    — a task ran and is already complete (the repo's own
     *                         name for this, lib/tasks.mjs, so there is one name
     *                         per idea rather than a second spelling of it)
     *   task-cancelled      — raised by the TURN once the harness reports it
     *
     * Sending cancellation is not observing termination: this returns what it
     * DID (the notification left), and the turn settles when the harness answers.
     */
    cancel() {
      if (stopped) throw fail("acp-closed", "the ACP client is closed");
      if (!session || turns === 0) throw fail("task-not-found", "no task has run in this session, so there is nothing to cancel");
      if (!prompt) throw fail("task-not-running", "the task is already complete; cancel applies to a running turn");
