...
export async function discoverHarnesses({ env = process.env, timeoutMs = 3000 } = {}) {
  ...
  const entries = await Promise.all(CLIS.map(async ([id, name, description, versionPattern]) => {
    const row = { id, name, description, command: id, state: "unknown", version: null,
      ...
      delegation: id === "pi" ? createPiAcpExecutor().check() : { ok: false, refused: "adapter-not-configured", why: "No Voicebox task adapter is configured for this CLI; configure an adapter before delegating." } };
    let file;
  ...
  const adapter = { id: "pi-acp", name: "Pi ACP adapter", command: "pi-acp", description: "Adapts Pi's coding agent to Agent Client Protocol.",
    state: "unknown", version: null, capabilities: "ACP adapter package; no handshake or model task tested by inventory.", delegation: createPiAcpExecutor().check() };
  try {
  ...
  return { ok: true, observedAt: new Date().toISOString(), scope: "Voicebox server machine only", entries,
    note: "Known CLI names in absolute PATH plus the Pi ACP package location; not an exhaustive machine scan. A matching version is not identity attestation or task readiness. No configured default, fleet discovery or model task is supplied. Pi's existing absent-capability broker refusal is an older code restriction awaiting alignment with independent-harness policy, not a new requirement to add a broker." };
}
