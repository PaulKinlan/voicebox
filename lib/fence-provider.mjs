...
export function measureBoundary(probe) {
  ...
  const networkMeasured = typeof parentLoopback?.ok === "boolean" || tcp.some((ok) => typeof ok === "boolean")
    || dnsOk || typeof network.dns?.error === "string";
  axes.network = {
    verdict: parentLoopback?.ok === true || outbound || dnsOk ? "passes" : networkMeasured ? "unknown" : "not measured",
    measured: networkMeasured,
    how: "probe: owned parent loopback marker, plus separate external TCP / DNS attempts",
    evidence: { parentLoopback, outbound, dns: dnsOk },
