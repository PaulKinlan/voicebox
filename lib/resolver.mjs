...
registerResolver("script", (transcript) => {
  ...
  return { unresolved: `the script resolver only knows create/read/list/delete/edit/diff/grep — got: "${transcript.slice(0, 80)}". Wire a model resolver to go further.` };
});
...
export function makeGeminiResolver({
  ...
  return async (transcript) => {
    ...
    if (!["list", "extensions", "grep"].includes(action.verb) && !action.name) {
