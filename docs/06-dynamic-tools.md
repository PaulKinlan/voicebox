...
coord. Companion to [`05-harvest.md`](05-harvest.md) — same method: paths and quoted source
...
`05-harvest.md` §3.1 concluded *pi is discovery, CAP is admission, and Voicebox only has the
...
- **Its memory is local Markdown files** — i.e. host-side, human-readable, reachable from every
  channel. That is independent evidence for the narrowing in `05-harvest.md` §2.6/§9.1: the
...
5. **What browser-only mode actually is.** If (a), it is a demo. If (b) or (c), it is a real
   ...
   one in `05-harvest.md` §9.1: **a separate world, not a cache**, until an explicit export
   runs.
...
Three amendments to `05-harvest.md`, listed rather than folded in so the earlier document stays
