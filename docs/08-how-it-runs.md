...
- **Interactive mini-apps rendered within the room interface.** The room embeds `#mini-app-container` hosting an outer mediator iframe pointing to `/mini-app-bridge.html?appId=...`. The bridge establishes a private `MessageChannel` with the room and sets up the untrusted inner app with `sandbox="allow-scripts"`. Conversational turns (`launch_mini_app`) or `{type:"mini_app"}` frames dynamically mount the widget with title, reload, collapse/expand, and close controls (voicebox-beads-5h1).
