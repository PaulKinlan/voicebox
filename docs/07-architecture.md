...
| **4 · the result returns** | the page gets the whole story in one response | `{transcript, action, result}` — `result.ok`, `result.action`, `result.root`, `result.logged` | → `ok: true`, `logged: 1` |
| **5 · the act is recorded** | one entry per act — allowed **or refused** — appended to the root's own log and readable back | `<root>/.audit/<writer>.jsonl` (`core/shared-log.ts`), `GET /api/audit` | → entry seq 1: kind `write`, decision `allow`, rule `writes-inside` |

**Where it fails, by name** (driven): the same turn **before any root is declared** → `refused: root-not-declared`, `logged: null` (no root, so nowhere to hold a log — the response says so rather than omitting the field); `"read .."` → `refused: outside-root`, and the refusal is itself logged as entry seq 2. Declaring the root answered `ok: true`, `reachableFromThisProcess: true`, and the turn that was refused a moment earlier then succeeded.
