...
- Choose **fieldnotes@box** to see two sessions, a request from the chat session that can be answered here, and a separately refusable merge. It is a different sample project, not a copy or sync of the browser project.
...
The output directory must not already exist. See [the interface proposal](../../docs/interface.md) for information architecture, alternative models and the exact source studies. The reviewed evidence receipt is linked there when available. No screenshots in this package establish a production runtime.
